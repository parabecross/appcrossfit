...
export default async function AdminRankingPage({
  ...
  const timezone = boxConfig.timezone;
  const currentMonth = currentMonthKeyInTimezone(timezone);
  const requestedMonth = isValidMonthKey(sp.month) ? sp.month : undefined;
  const monthKey = requestedMonth ?? currentMonth;
  const monthLabel = formatMonthKeyLabel(monthKey, locale);

  const [config, data, awards, availableMonths] = await Promise.all([
